...
#define UART_RX_DMA_BUFFER_SIZE    (2048U)
...
ATTR_PLACE_AT_NONCACHEABLE_BSS_WITH_ALIGNMENT(32)
...
USB_NOCACHE_RAM_SECTION byte_queue_t com_uartrx;
USB_NOCACHE_RAM_SECTION byte_queue_t com_usbrx;

static ATTR_PLACE_AT_FAST_RAM_NON_INIT_WITH_ALIGNMENT(32) uint8_t uartrx_ringbuffer[CONFIG_UARTRX_RINGBUF_SIZE];
static ATTR_PLACE_AT_FAST_RAM_NON_INIT_WITH_ALIGNMENT(32) uint8_t usbrx_ringbuffer[CONFIG_USBRX_RINGBUF_SIZE];
...
static ATTR_PLACE_AT_FAST_RAM_NON_INIT_WITH_ALIGNMENT(32) uint8_t usb_rx_tmpbuffer[CONFIG_USBRX_RINGBUF_SIZE];
static ATTR_PLACE_AT_FAST_RAM_NON_INIT_WITH_ALIGNMENT(32) uint8_t uart_tx_tmpbuffer[CONFIG_UARTRX_RINGBUF_SIZE];

static void format_timestamp(uint8_t direction, size_t timeOutms)
{
    char chTimeStamp[20];

    static int64_t lTimeStampTxold = 0,lTimeStampRxold = 0;
    int64_t lTimeStamp = get_system_ms();

    uint32_t ms = lTimeStamp % 1000;
    uint32_t total_sec = lTimeStamp / 1000;
    uint32_t sec = total_sec % 60;
    uint32_t min = (total_sec / 60) % 60;
    uint32_t hour = (total_sec / 3600) % 24;
    memset(chTimeStamp, 0, sizeof(chTimeStamp));

    if(direction == 0){
        if(lTimeStamp - lTimeStampRxold > timeOutms){
            lTimeStampRxold  = lTimeStamp;
            snprintf((char *)chTimeStamp, sizeof(chTimeStamp), "[%02u:%02u:%02u.%03u]%s",
                     hour, min, sec, ms, "<-\n");
            emit(uart_rx_time_sig, &tUartMsgObj,
                 args(
                      chTimeStamp,
                      strlen(chTimeStamp)
                 ));
        ...
        if(lTimeStamp - lTimeStampTxold > timeOutms){
            lTimeStampTxold  = lTimeStamp;
            snprintf((char *)chTimeStamp, sizeof(chTimeStamp), "[%02u:%02u:%02u.%03u]%s",
                     hour, min, sec, ms, "->\n");
            emit(uart_tx_time_sig, &tUartMsgObj,
                 args(
                      chTimeStamp,
                      strlen(chTimeStamp)
                 ));
...
{
    byte_queue_init(&com_uartrx, uartrx_ringbuffer, CONFIG_UARTRX_RINGBUF_SIZE);
    byte_queue_init(&com_usbrx, usbrx_ringbuffer, CONFIG_USBRX_RINGBUF_SIZE);
    connect(&tUartMsgObj, SIGNAL(uart_sig), &com_uartrx, SLOT(enqueue_bytes));  
...
static void chry_dap_usb2uart_uart_config_callback(struct cdc_line_coding *line_coding)
{
    ...
    config.rxidle_config.threshold = 1U; /* 1bit */
    ...
    config.txidle_config.threshold = 1U;
...
static void usbd_cdc_acm_bulk_out(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    (void) busid;
    (void) ep;
    enqueue_bytes(&com_usbrx, cdc_tmpbuffer, nbytes);
    if (get_queue_available_count(&com_usbrx) >= DAP_PACKET_SIZE) {
...
static void usbd_cdc_acm_bulk_in(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    if ((nbytes % DAP_PACKET_SIZE) == 0 && nbytes) {
        ...
        size = get_queue_count(&com_uartrx);
        if (size) {
            size = dequeue(&com_uartrx, usb_rx_tmpbuffer,size);
            usbd_ep_start_write(0, ep, usb_rx_tmpbuffer, size);
        } else {
            usbtx_idle_flag = 1;
        }
    }
...
static void chry_dap_usb2uart_uart_send_complete(uint32_t size)
{
    uint32_t count = get_queue_count(&com_usbrx);
    if (count) {
        size = dequeue(&com_usbrx, uart_tx_tmpbuffer,count);
        chry_dap_usb2uart_uart_send_bydma(uart_tx_tmpbuffer, size);
    } else {
        uarttx_idle_flag = 1;
    }
}
    ...
    */
    ...
    if (usbtx_idle_flag) {
        ...
        if (size) {
            ...
            size = dequeue(&com_uartrx, usb_rx_tmpbuffer,size);
            usbd_ep_start_write(0, CDC_IN_EP, usb_rx_tmpbuffer, size);
        }
    }

    /* usbrx to uart tx */
    if (uarttx_idle_flag) {
        size = get_queue_count(&com_usbrx);
        if (size) {
            uarttx_idle_flag = 0;
            /* start first transfer */
            size = dequeue(&com_usbrx,uart_tx_tmpbuffer, size);
            chry_dap_usb2uart_uart_send_bydma(uart_tx_tmpbuffer, size);
        }
    }
